...
public class FaqsController {
    ...
    @Autowired
    private IFaqsService faqService;

    // Create a new FAQ
    @PostMapping("/create")
    public ResponseEntity<Faqs> create(@RequestBody Faqs faq) {
        Faqs createdFaq = faqService.create(faq);
    ...
    public ResponseEntity<Faqs> update(@RequestBody Faqs faq) {
        Faqs updatedFaq = faqService.update(faq);
    ...
    public ResponseEntity<List<Faqs>> getAll() {
        List<Faqs> allFaq = faqService.getAll();
